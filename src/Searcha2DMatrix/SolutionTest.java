package Searcha2DMatrix;

import org.junit.jupiter.api.Test;
import org.junit.jupiter.params.ParameterizedTest;
import org.junit.jupiter.params.provider.Arguments;
import org.junit.jupiter.params.provider.MethodSource;

import java.util.stream.Stream;

import static org.junit.jupiter.api.Assertions.*;

class SolutionTest {
    private Solution solution = new Solution();


    @ParameterizedTest
    @MethodSource("params")
    void test(int[][] matrix, int target, boolean expected) {
        assertEquals(expected, solution.searchMatrix(matrix, target));
    }

    @ParameterizedTest
    @MethodSource("params")
    void testSingleBinSearch(int[][] matrix, int target, boolean expected) {
        assertEquals(expected, solution.searchMatrixSingleBinSearch(matrix, target));
    }


    private static Stream<Arguments> params() {
        return Stream.of(
                Arguments.of(new int[][]{{1, 3, 5, 7}, {10, 11, 16, 20}, {23, 30, 34, 60}}, 3, true),
                Arguments.of(new int[][]{{1, 3, 5, 7}, {10, 11, 16, 20}, {23, 30, 34, 60}}, 13, false),
                Arguments.of(new int[][]{{1}}, 2, false),
                Arguments.of(new int[][]{{1, 3}}, 3, true),
                Arguments.of(new int[][]{{1}, {3}, {5}}, 3, true)
        );
    }


}